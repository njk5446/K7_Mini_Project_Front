import React, { useState } from "react" // React 라이브러리에서 React와 useState 훅을 가져오는 것
// useState: const[변경할 변수, 변경할값] = useState("");
import { Link, useNavigate } from "react-router-dom"; //react-router-dom 라이브러리에서 Link, useNavigate 가져오는 것

const Login = () => {
    const [email, setEmail] = useState(""); // ("")는 초기값 설정
...
// 서버 로그인 처리
const handleLogin = async (e) => { // 비동기: 코드가 실행되는 동안 다른 작업을 동시에 진행하는 방식
    // async(이벤트 파라미터): 비동기작업 요청 시 handleLogin함수 호출                               
    e.preventDefault(); // 비동기작업요청(e).preventDefault: 비동기작업요청하면 새로고침을 막는다.
    ...
    // 서버 요청의 응답
    const response = await fetch( // fetch: 서버에 HTTP 요청을 보내는 함수
        ...
        "로그인 서버 주소", // 요청을 보낼 서버의 URL 
        {
            ...
            body: JSON.stringify({
                 // JSON.stringify를 통해 email과 password를 JSON 문자열로 변환
                email: email,
                password: password,
            }),
        }
    ...
    if (response.status == 200) { // 200(성공)인지 응답 상태 확인
        setLoginCheck(false);  // 로그인 성공하면 loginCheck가 필요없으니까 false
